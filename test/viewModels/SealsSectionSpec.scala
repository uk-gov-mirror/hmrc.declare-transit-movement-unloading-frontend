...
class SealsSectionSpec extends SpecBase {
  ...
  "SealsSection" - {
    ...
    "contain data from unloading permission" in {
      ...
      val data: Seq[Section] = SealsSection(emptyUserAnswers, 2)(withSeals, new UnloadingSummaryRow(emptyUserAnswers))
    ...
    "contain data from user answers" in {
      ...
      val data: Seq[Section] = SealsSection(updatedUserAnswers, 2)(withSeals, new UnloadingSummaryRow(updatedUserAnswers))
    ...
    "return nothing if no seals exist" in {
      ...
      val data: Seq[Section] = SealsSection(emptyUserAnswers, 0)(noSeals, new UnloadingSummaryRow(emptyUserAnswers))
      data mustBe Nil
    }
